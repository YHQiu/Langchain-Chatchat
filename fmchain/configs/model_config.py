...
# 日志存储路径
BASE_DATA_PATH = "/data/langchain-chatchat"
os.makedirs(BASE_DATA_PATH, exist_ok=True)
LOG_PATH = os.path.join(BASE_DATA_PATH, "logs")
if not os.path.exists(LOG_PATH):
    os.mkdir(LOG_PATH)

# 知识库默认存储路径
KB_ROOT_PATH = os.path.join(BASE_DATA_PATH, "knowledge_base")
...
# 获取单个环境变量的值
SQLALCHEMY_DATABASE_URI_VALUE = os.environ.get('SQLALCHEMY_DATABASE_URI')
# 如果环境变量存在，则 variable_value 将包含其值；否则，它将为 None
if SQLALCHEMY_DATABASE_URI_VALUE is not None:
    SQLALCHEMY_DATABASE_URI = SQLALCHEMY_DATABASE_URI_VALUE
else:
    SQLALCHEMY_DATABASE_URI = f"mysql://root:123456@127.0.0.1:3306/chatchat"
